...
public class NVPAPICaller
{
    ...
    public bool ShortcutExpressCheckout(string amt, ref string token, ref string retMsg)
    {
        ...
        using (ShoppingCartActions myCartOrders = new ShoppingCartActions(_httpContextAccessor,_context))
        {
            List<CartItem> myOrderList = myCartOrders.GetCartItems();

            for (int i = 0; i < myOrderList.Count; i++)
            {
                encoder["L_PAYMENTREQUEST_0_NAME" + i] = myOrderList[i].Product.Description.ToString();
                encoder["L_PAYMENTREQUEST_0_AMT" + i] = myOrderList[i].Product.SellingPrice.ToString();
    ...
    public string HttpCall(string NvpRequest)
    {
        ...
        WebRequest objRequest = (HttpWebRequest)WebRequest.Create(url);
